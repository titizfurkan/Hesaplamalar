...
export default class IhtimalHesaplama extends Component {
  constructor(props) {
    super(props);
    this.state = {
      input1: "",
  ...
  sonucMethod() {
    ...
    var sonuc = number1 / (number1 + number2) * 100; 
    sonuc = (sonuc.toFixed(2))
    var sonuc2 = number2 / (number1 + number2) * 100;
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.contentWrapper}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Başarısızlık Sayısı"
            onChangeText={(text) => {
              this.setState({
                input2: text,
              });
          ...
          <Text style={styles.sampleText}> Başarı İhtimali : {this.state.sonuc}%</Text>
          <Text style={styles.sampleText}> Başarısızlık  İhtimali : {this.state.sonuc2}%</Text>
        </View>
      </SafeAreaView>
    );
  }
